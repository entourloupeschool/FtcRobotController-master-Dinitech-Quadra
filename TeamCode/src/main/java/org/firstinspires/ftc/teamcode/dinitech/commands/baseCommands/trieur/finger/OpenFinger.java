package org.firstinspires.ftc.teamcode.dinitech.commands.baseCommands.trieur.finger;

import com.arcrobotics.ftclib.command.CommandBase;

import org.firstinspires.ftc.teamcode.dinitech.subsytems.TrieurSubsystem;

/**
 * An instant command that opens the finger on the sorter.
 * <p>
 * When scheduled, this command calls the {@code openFinger()} method on the
 * {@link TrieurSubsystem} and finishes immediately.
 */
public class OpenFinger extends CommandBase {
    private final TrieurSubsystem trieurSubsystem;

    /**
     * Creates a new OpenFinger command.
     *
     * @param trieurSubsystem The sorter subsystem to control.
     */
    public OpenFinger(TrieurSubsystem trieurSubsystem){
        this.trieurSubsystem = trieurSubsystem;
        addRequirements(trieurSubsystem);
    }

    /**
     * Opens the finger.
     */
    @Override
    public void initialize(){
        trieurSubsystem.openFinger();
    }

    /**
     * This command is always finished immediately.
     *
     * @return Always returns true.
     */
    @Override
    public boolean isFinished() {
        return true;
    }
}